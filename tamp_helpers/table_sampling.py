...
def sample_on_table(table_id, env, z_offset=0.1):
  ...
  table_entity = pddl_id_to_mujoco_entity(table_id, env)
...
def is_pos_colliding_with_block(block_id, sampled_pos, env, padding=0.07):
  """
  ...
  block_entity = pddl_id_to_mujoco_entity(block_id, env)
...
def is_pos_colliding(block_id, sampled_pos, env, skip_self=True):
  """
  ...
  for other_block_id in BLOCK_PDDL_IDS:
    ...
    if is_pos_colliding_with_block(other_block_id, sampled_pos, env):
      return True
...
def sample_free_spot_on_table_for_block(table_id, block_id, env, max_attempts=1_000, skip_self=True):
  """
  ...
  attempt = 0
  sampled_pos = sample_on_table(table_id, env)

  # continue sampling until a good position is found or until the maximum number of attempts
  # is reached.
  while is_pos_colliding(block_id, sampled_pos, env, skip_self) and attempt < max_attempts:
    sampled_pos = sample_on_table(table_id, env)
    attempt += 1
